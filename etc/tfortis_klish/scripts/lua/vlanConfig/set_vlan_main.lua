...
function getUciInterfaceLan()
    ...
    mainVlan.device = result[1]
    mainVlan.id     = result[2]
end

function printVlanByKey(key)
    key = tostring(key)
    local brVlan = brVlanList[key]
    print("==================================")
    print("key = ", key)
    print("id",   brVlan.id)
    print("device", brVlan.device)
    print("index",  brVlan.index)

    if brVlan.ports == nil then
        print("ports not configured")
    else
        print("Ports: ")
        for  num, port in pairs(brVlan.ports) do
            --print("port num = " .. num .. " id = " .. port.id .. " State = " .. port.state)
            print("port num = " .. num .. " port = " .. port)
        end
    end
end

function printBrVlanInfo()
    for i, brVlan in pairs(brVlanList) do
...
function getPort(portStr)
    ...
    local portStr = tostring(portStr)
    if string.find(portStr, ":t") then
        local str, num, isTagget = portStr:match("(%w+)(%d+):(%a+)")
        portNumber = tonumber(num)
        state = "tagged"
    else
        local str, num = portStr:match("(%a+)(%d+)")
        portNumber = tonumber(num)
    end
    port.state      = state
...
function saveUntaggedPorts()
    for _, vlan  in pairs(brVlanList) do
        local busyList = {}
...
function vlan_add_notMemberPorts()
    ...
    local key_in = vlanRange[1]
    key_in = tostring(key_in)
    local thisVlan = brVlanList[key_in]
